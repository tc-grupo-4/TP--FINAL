...
class EtapaEA: #una etapa que puede ser tan compleja como se desee
    minGain = None
    maxGain = None
    name = None
    ...
    def getMaxMinGain(self):
        w, mag, pha = signal.bode(self.tf, logspace(log10(config.min_freq*2*pi), log10(config.max_freq*2*pi), 10000))
        self.mag = mag
        self.w = w
        ...
        self.minGain = minGain + self.gain
        self.maxGain = maxGain + self.gain
